<%@page import="osa.ora.iot.db.beans.ActionsLov"%>
...
<title>SensorWays IoT, View Audit Records</title>
...
<body>
	...
	&nbsp;&nbsp;<a href='main.jsp'>Main Menu</a> > View Audit
        <h3>&nbsp;View Recent Audit Records</h3>
        <form method="post" action="IoTWebServlet">
            <input type="hidden" name="action" value="<%=IConstant.ACTION_WEB_LIST_AUDIT%>"/>
            <table>
            <tr><td>Max Displayed Audit Records 
                <select name="rows"/>
                ...
                </select> Filter Specific Action 
                <select name="actionFilter"/>
                  <option value="1000" selected>All Actions</option>
                    <% 
                         List<ActionsLov> actionsList = (List<ActionsLov>)session.getAttribute("ACTIONS"); 
                         for(int i=0;i<actionsList.size();i++){
                             out.println("<option value='"+actionsList.get(i).getActionId()+"'>"+actionsList.get(i).getActionsName()+"</option>");
                         }
                    %>
                </select> <input  name="submitButton" value="Refresh" type="submit"/>
                </td></tr>
            </table> 
        </form>
	<table>
	<tr><th>Action</th><th>By</th><th>On</th><th>Parameters</th></tr>
	<%
        List<Object> auditRecords = null;
	if(session.getAttribute("AUDIT_RECORDS")!=null){
		auditRecords = (List<Object>)session.getAttribute("AUDIT_RECORDS");
	for (Iterator i = auditRecords.iterator(); i.hasNext();) {
            Object[] values = (Object[]) i.next();
            %>
            <tr><td><%=values[2]%>
            </td><td><%=values[1]%>
            </td><td><%=values[3]==null?"N/A":formatter.format(values[3])%>
            </td><td><%=values[4]%>
            </td></tr>
	<%}
        }%>
